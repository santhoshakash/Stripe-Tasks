    ...
    if (latest_invoice.payment_intent) {
      ...
      if (status === "requires_action") {
        const { error: confirmationError } = await stripe.confirmCardPayment(
          client_secret
          //this will show a prompt for 3d secure
        );
      ...
      <AuthCheck fallback={<SignIn />}>
        ...
        <div className="well">
          ...
          <button
            className={
              "btn " +
              (plan === "price_1KZVWlSDTwncXAc7lH8Crj4i"
                ? "btn-primary"
                : "btn-outline-primary")
            }
            onClick={() => setPlan("price_1KZVWlSDTwncXAc7lH8Crj4i")}
          >
            Choose Monthly 20 INR/m
          </button>
